...
Проверка при экипировке использует **итоговые** статы ОВ (база + экипировка + пассивы `main_stats_flat` + flat-бонусы Paragon/совершенствования + `all_stats_pct` по той же формуле, что `/profile`), включая `charm` и `luck`. При надевании в конкретный слот симулируется замена предмета в этом слоте (бонусы снимаемой вещи не учитываются); пилюли требований (`requirements_status`) для этой проверки отражают статы **после замены**. Превью без целевого слота — по текущей экипировке без кандидата. Реализация: `game/equip_requirements.py`.
