...
# Экспедиции: требования «живого» ритма перед генерацией черновика.
AI_NARRATIVE_HUMAN_RHYTHM_REQUIREMENTS_RU = (
    "Большинство ИИ-текстов пишутся предсказуемым ритмом: "
    "предложения одинаковой длины, структура повторяется, темп механический. "
    "Пиши с естественным человеческим ритмом.\n"
    "Требования:\n"
    "- Чередуй короткие и длинные предложения\n"
    "- Иногда используй обрывки фраз\n"
    "- Меняй длину абзацев\n"
    "- Избегай повторяющихся начал\n"
    "- Создавай контраст и напряжение\n"
    "- Убери всё, что ощущается механически оптимизированным\n"
    "Текст должен звучать живым, а не сгенерированным."
)

# Экспедиции: промпт проверки черновика на «типично-ИИ» формулировки.
AI_NARRATIVE_ANTI_GENERIC_REVIEW_PROMPT_RU = (
    "Проанализируй текст ниже. Выдели каждое предложение, которое могло бы легко "
    "встретиться в тысячах ИИ-статей. Для каждого такого предложения:\n"
    "1. Объясни, почему оно кажется шаблонным.\n"
    "2. Перепиши его с более оригинальным наблюдением.\n"
    "3. Добавь конкретики.\n"
    "4. Добавь более ясную перспективу.\n"
    "Затем дай полностью переписанную версию — отличимую и запоминающуюся.\n"
    "В самом конце ответа, после всего анализа, выведи только финальный нарратив "
    "после маркера ===ИТОГ=== — без пояснений после него."
)
