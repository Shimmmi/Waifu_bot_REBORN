...
AI_NARRATIVE_ANTI_GENERIC_VERIFY_RU = (
    "Перепиши текст экспедиции так, чтобы он звучал живым и запоминающимся, без шаблонных ИИ-фраз. "
    "Мысленно проверь каждое предложение: не generic ли оно, достаточно ли конкретики и ясной перспективы. "
    "Анализ выполни мысленно — не выводи его в ответе.\n\n"
    "Текст:\n{draft}\n\n"
    "Формат ответа (строго): одна строка ===ФИНАЛ===, затем с новой строки только переписанный текст на русском. "
    "Без списков, без пояснений, без markdown, без слова «generic», без разбора предложений."
)
