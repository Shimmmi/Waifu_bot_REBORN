...
/** URL-ы фона и торговца магазина по акту. */
function shopStageImageUrls(act) {
  const a = Math.max(1, Math.min(5, safeInt(act, 1)));
  return {
    bg: [
      `/static/shop/act-${a}/background.webp`,
      `/static/shop/bg_act${a}.webp`,
      `/static/shop/shop.background.webp`,
    ],
    merchant: [
      `/static/shop/act-${a}/merchant.webp`,
      `/static/shop/merchant_act${a}.webp`,
      `/static/shop/shop.merchant.webp`,
    ],
  };
}

function applyShopStageImages(act) {
  const a = Math.max(1, Math.min(5, safeInt(act, 1)));
  const bgImg = document.getElementById("shop-bg-img");
  const merchantImg = document.getElementById("shop-merchant-img");
  const merchantVisual = merchantImg?.closest?.(".shop-merchant-visual");
  const fallback = merchantVisual?.querySelector?.(".fallback");
  const urls = shopStageImageUrls(a);

  if (merchantVisual) merchantVisual.classList.remove("merchant-fallback");

  if (bgImg) {
    bgImg.style.display = "";
    attachCaravanImage(bgImg, urls.bg, () => {
      bgImg.style.display = "none";
    });
  }

  if (merchantImg && merchantVisual) {
    merchantImg.style.display = "";
    if (fallback) fallback.style.display = "none";
    attachCaravanImage(merchantImg, urls.merchant, () => {
      merchantImg.style.display = "none";
      if (fallback) fallback.style.display = "";
      merchantVisual.classList.add("merchant-fallback");
    });
  }
}
