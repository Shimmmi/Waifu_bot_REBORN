...
from waifu_bot.services.ai_narrative_rewrite import escape_telegram_html
...
def build_gd_folding_chronicle(
    ...
    *,
    ...
    fallen_names: list[str] | None = None,
    max_chars_each: int = 280,
) -> str:
    """Compact prior-round story for folding (not full dumps)."""
    import re

    parts: list[str] = ["ХРОНИКА ПОХОДА (продолжи, не повторяй те же шутки/образы):"]
    facts = []
    if wave is not None:
        facts.append(f"волна={wave}")
    if round_num is not None:
        facts.append(f"раунд={round_num}")
    if wipe_count:
        facts.append(f"нокауты_отряда={wipe_count}")
    if last_outcome:
        facts.append(f"прошлый_исход={last_outcome}")
    if fallen_names:
        facts.append("недавно_падали: " + ", ".join(fallen_names[:4]))
    if facts:
        parts.append("Факты: " + "; ".join(facts) + ".")
    cleaned: list[str] = []
    for raw in recent_narratives[:2]:
        t = re.sub(r"<[^>]+>", "", str(raw or ""))
        t = re.sub(r"\s+", " ", t).strip()
        if not t:
            continue
        if len(t) > max_chars_each:
            t = t[: max_chars_each - 1].rstrip() + "…"
        cleaned.append(t)
    if cleaned:
        for i, t in enumerate(cleaned, 1):
            parts.append(f"• Ранее ({i}): {t}")
    else:
        parts.append("• Предыдущих сцен ещё нет — задай тон похода.")
    return "\n".join(parts)
...
def build_user_prompt_round(ctx: dict[str, Any]) -> str:
    ...
    chronicle = (ctx.get("folding_chronicle") or "").strip()
    if chronicle:
        lines.append(chronicle)
    lines.append("СОСТАВ ОТРЯДА:")
    lines.append(
    ...
    from waifu_bot.services.gd_narrative_seeds import format_seed_and_fingerprint_prompt_block

    lines.append(
        format_seed_and_fingerprint_prompt_block(
            ctx.get("absurd_event_seed"),
            list(ctx.get("recent_narrative_fingerprints") or []),
        )
    )
    if ctx.get("power_hint"):
        lines.append(
            f"Намёк на силу билда (без цифр): {ctx.get('power_hint')}."
        )
    return "\n".join(lines)
...
def build_user_prompt_finale(ctx: dict[str, Any]) -> str:
    ...
    lines.append(GD_NARRATIVE_FORMATTING_RU)
    mvp_power = ctx.get("mvp_power_name")
    mvp_presence = ctx.get("mvp_presence_name")
    if mvp_power or mvp_presence:
        lines.append(
            "Два героя финала (если разные): "
            f"MVP силы/вклада в бой — {mvp_power or '—'}; "
            f"MVP присутствия в чате — {mvp_presence or '—'}."
        )
    from waifu_bot.services.gd_narrative_seeds import format_seed_and_fingerprint_prompt_block
    ...
    lines.append(
        ...
        "Если указаны два MVP (сила и присутствие) — обыграй обоих. Без цифр."
    )
    return "\n".join(lines)
...
async def generate_gd_daily_finale_narrative(
    ...
    *,
    ...
    try:
        ...
        logger.exception("GD daily finale narrative failed")
        return None, stub
