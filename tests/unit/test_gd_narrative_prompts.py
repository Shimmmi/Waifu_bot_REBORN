...
def test_system_prompt_no_unconditional_silent_jokes() -> None:
    ...
    assert "<b>" in GD_SYSTEM_PROMPT
    assert "2–3 абзаца" in GD_SYSTEM_PROMPT


def test_round_prompt_has_html_formatting_block() -> None:
    party = _solo_party()
    ctx = {
        ...
        "actions": [{"user_id": 305174198, "kind": "skill", "skill": "DAMAGE_SINGLE"}],
        "flags": {},
        "raw_buffer_users": {},
        "outcomes_summary": {},
    }
    prompt = build_user_prompt_round(ctx)
    assert "НАВЫКИ И ЭФФЕКТЫ" in prompt
    assert "(урон)" in prompt
    assert "<b>Алиса</b>" in prompt
    block = build_gd_actions_format_block(ctx)
    assert "DAMAGE_SINGLE" not in block
    assert "(урон)" in block


def test_round_prompt_includes_damage_amounts() -> None:
    party = _solo_party()
    ctx = {
        "dungeon_name": "Пустыня",
        "biome_tag": "песок",
        ...
        "monsters": [{"name": "Скорпион", "level": 5, "hp": 50, "max_hp": 100}],
        "actions": [
            {"kind": "cycle_start", "cycle": 1},
            {"user_id": 305174198, "kind": "text", "cycle": 1, "series": 3, "damage": 42},
            {"user_id": 305174198, "skill": "DAMAGE_SINGLE", "damage": 77},
        ],
        "flags": {},
        "raw_buffer_users": {},
        "outcomes_summary": {},
    ...
    prompt = build_user_prompt_round(ctx)
    assert "урон 42" in prompt
    assert "серия из 3" in prompt
    block = build_gd_actions_format_block(ctx)
    assert "урон 77" in block


def test_round_prompt_victory_includes_mvp() -> None:
    party = _solo_party()
    ctx = {
        "dungeon_name": "Пустыня",
        "biome_tag": "песок",
        ...
        "mvp_name": "Алиса",
        "party": party,
        "monsters": [{"name": "Скорпион", "level": 5, "hp": 0, "max_hp": 100}],
        "actions": [{"user_id": 305174198, "kind": "text", "cycle": 1, "series": 1, "damage": 99}],
        "flags": {},
        "raw_buffer_users": {},
        "outcomes_summary": {},
    }
    prompt = build_user_prompt_round(ctx)
    assert "ПОБЕДА" in prompt
...
def test_solo_round_prompt_forbids_invented_allies() -> None:
    ...
    assert "Алиса (user 305174198): текстовая атака, урон 20" in prompt
